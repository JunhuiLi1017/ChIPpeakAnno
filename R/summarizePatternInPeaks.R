...
#' 'motif_occurence'. The 'motif_enrichment' has four columns: 
#' \itemize{
...
#' }
#' 
#'The 'motif_occurence' has 14 columns:
#'
#' \itemize{
...
patternEnrichmentInPeak <- function(patternVec,
  ...
  if(method == "binom.test"){
    peakPvalue <- binomEnrichment(pattern, expectFrequencyMethod=
                                  expectFrequencyMethod, revcomp=revcomp, 
                                  seqs = seqs, MarkovOrder=MarkovOrder)
  }else if(method == "permutation.test"){
    peakPvalue <- permutationEnrichment(pattern, revcomp=revcomp, 
                                  seqs = seqs, genome=BSgenomeName, 
                                  backgroud = bgdForPerm, 
                                  nperm=nperm, alpha=alpha)
  }
  return(peakPvalue)
}

binomEnrichment <- function(pattern, expectFrequencyMethod=c("Naive","Markov"), 
                            revcomp=TRUE, seqs, MarkovOrder=3L){
...
expectMotifFrequency <- function(pattern, 
  ...
  if(expectFrequencyMethod == "Naive"){
    ...
    stopifnot("MarkovOrder should less than motif length - 2"=
...
permutationEnrichment <- function(pattern, 
  ...
  permutationFreq <- do.call(rbind,lapply(seq.int(nperm),function(n) {
...
getPosInSeqs <- function(thispattern, seq, revcomp = TRUE){
  ...
  if (length(total) == 0){
    ...
    total <- cbind(motifChr=motifstartInChromosome,motifStartInChr =
                     motifstartInChromosome, motifEndInChr = 
                     motifendInChromosome, total)
    colnames(total)[(ncol(total)-4):ncol(total)] <- c("peakChr","peakStart",
